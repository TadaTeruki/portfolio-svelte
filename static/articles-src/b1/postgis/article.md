---
title: "北海道の産業施設をPostGISで眺める" 
description: "「自動車等試験研究のまち・士別市」"
thumbnail: "muroran.webp"
created_at: "2025-1-24"
---

# はじめに

今年度履修していたDB工学の講義にて自由課題があったため、PostGISに入門しました。課題の提出期限を過ぎたため、自分の提出内容を共有します。

## テーマ

![苫小牧市の工業団地](./images/tomakomai.webp)

北海道の多様な産業施設について、その立地や施設名を管理するDBを作成します。
...
また`INDEX`と同じ要領で[空間インデックスを張れる](https://postgis.net/docs/using_postgis_dbmanagement.html#build-indexes)のも面白い点です。

## 基本的な流れ

今回はあくまで簡単なデモのため、気軽に利用できるOpenStreetMapの生のデータから抽出したものを利用します。

### Overpass APIから所定のデータを取得
...
この他にも、市町村と振興局の境界もそれぞれ独立に取得します。これらの結合は、後述のPostGISの提供する諸々の空間クエリで行います。

### DB立ち上げ

今回はDockerでPostGISのコンテナを立ち上げました。[公式のイメージ](https://github.com/postgis/docker-postgis)があるのはありがたい話です。

### GDAL/OGRでデータをPostGISに取り込む

GeoJSONをテーブルとして取り込みます。OpenStreetMapのタグがそのままカラムになっているのが良い感じです。DBは環境変数や`createdb`等で事前に立ち上げておく必要があるようです。
...
![直接importしたOSMデータのカラム。多くのタグは実データ上でNULLとなっていることが予想されるので、整理が必要です](./images/columns.webp)

### テーブル設計と実装

産業施設テーブル・市町村テーブル・振興局テーブルを作成し、それぞれの関係を定義します。テーブル設計は以下の通りです。
...
![北海道全体での産業施設の立地。各境界線は振興局・総合振興局の境界(赤)および市町村境界(緑)。全体を通してみると、胆振地方に地物が集中しているように見えます](./images/all.webp)

![室蘭市の産業施設。製鉄所が美しいです](./images/muroran.webp)

![上川地域北部の産業施設。自動車関連の試験場が目立ちます](./images/shibetsu_wide.webp)
...
自動車関連の試験場が上位を占める結果でした。詳しく調べた所、士別市は「自動車等試験研究のまち・士別市」として街をPRしているらしい。
...
地図を眺めていると、確かに贅沢に土地を使って堂々と立地しています。

![士別市の自動車関連の試験場。特に左のトヨタとダイハツの試験場が目立ちます。他にマツダが近隣にあるほか、細長いものはヤマハの試験場のようです](./images/shibetsu.webp)

> 特に北海道北部の内陸部は、寒冷地で積雪量も多く、広い土地が比較的容易に確保できることから、寒冷地試験の適地として試験場の造成が進められ、現在、道内の自動車関連試験場は28を数えます。

こう、過酷な環境がアドバンテージになっているのは興味深い話です。周辺には他にも、下川や仁宇布に試験場が見られました。
...
上川地方が最も面積が広いことがわかります。これも自動車関連の試験場の多さによるものと思われますが、少々想定外の結果ではあります。ただしこの尺度に置いても、苫小牧・室蘭・白老を抱える胆振地方は強く、上川地方と条件の近い十勝地方を抑えて二番目に位置しています。

![留萌や檜山地域は控えめですが、風力発電は目ぼしいものです](./images/rumoi.webp)
...
(市町村名)|(振興局名)|(面積合計, 平方メートル)|(最大の産業施設名)

![](./images/4.webp)

自動車関連の試験場の他に特徴的なものとしては、製鉄所(室蘭)や製紙工場(旭川・白老)のある都市が目立ちます。ほかは工業団地や発電所、石油備蓄基地といった施設を抱える都市が多いです。
...
![北斗市。西側の峩朗鉱山(石灰石)や海に伸びるベルトコンベアが目立ちます](./images/hokuto2.webp)

このあたりのセメント産業については「北斗市歴史年表」(https://www.city.hokuto.hokkaido.jp/bunkazai/data/sement.htm 、北斗市のサイトのようですが更新されているかは怪しく、各所に味が出ています)に詳しい話があります。最盛期には専用の電気鉄道もあったようです。

## おわりに

厳密な調査では無いので必ずしも正確とは言えないものですが、課題にかこつけた技術調査がメインのため、これでも十分楽しめる結果です。

空間インデックスもしかり、空間的な内包関係による絞り込みを`WHERE`句で処理するといった具合に、空間タスクをSQLにうまく落とし込んでいるのが良い感触でした。
